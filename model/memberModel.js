...
var memberDB = {
        ...
        return new Promise((resolve, reject) => {
            ...
            conn.connect(function (err) {
                if (err) {
                    ...
                    // ju : remove any password related bcuz change password has been moved to another tab!
    ...
    updateMemberPassword: function (details) {
        return new Promise((resolve, reject) => {
            var conn = db.getConnection();
            conn.connect(function (err) {
                if (err) {
                    ...
                    console.log("hiii im at update password")
                    var password = details.password;
                    var email = details.email;

                    bcrypt.hash(password, 5, function (err, hash) {
                        var sql = 'UPDATE memberentity SET PASSWORDHASH=? WHERE EMAIL=?';
                        var sqlArgs = [hash, email];
                        conn.query(sql, sqlArgs, function (err, result) {
                            if (err) {
                                conn.end();
                                return reject(err);
